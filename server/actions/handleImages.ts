...
import { db } from ".."
import {  eImage, mImages } from "../schema"
...
export async function handleEImage(images:{url:string, name:string, key:string}[], eventId:number){
  try {
    const imageInserts = images.map((image) =>({
      eventId,
      imageUrl:image.url,
